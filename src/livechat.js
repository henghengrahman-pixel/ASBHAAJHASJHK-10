...
export class LiveChatClient {
  ...
  async ensureRequesterInChat(chatId) {
    ...
    const addMembership=async(visibility)=>this.call('add_user_to_chat',{
      chat_id:id,
      user_id:userId,
      user_type:'agent',
      visibility,
    ...
    try{
      await addMembership('all');
      return {ok:true,added:true,userId,visibility:'all'};
    }catch(err){
      ...
      if(this.isPublicAgentLimitError(err)){
        // Do not evict or modify an existing human agent. A chat can be at the provider's
        // public-agent capacity while still accepting an agent-visible participant. Add the
        // PAT owner privately, then retry the original public event from sendEvent(). This
        // preserves the existing human assignment and only changes the failing capacity path.
        try{
          await addMembership('agents');
          return {ok:true,added:true,userId,visibility:'agents',publicAgentLimitRecovered:true};
        }catch(privateErr){
          if(this.isChatInactiveError(privateErr)) throw this.inactiveChatError(privateErr);
          if(this.isAlreadyChatUserError(privateErr)) return {ok:true,added:false,alreadyPresent:true,userId,visibility:'agents',publicAgentLimitRecovered:true};
          if(Number(privateErr?.status)===422 && /user_id.*not found|user.*not found/i.test(String(privateErr?.message||privateErr||''))){
            const er=new Error(`LIVECHAT_REQUESTER_USER_ID_INVALID: '${userId}' is not a valid LiveChat agent user_id for this license; set LIVECHAT_REQUESTER_USER_ID to the exact agent email/user id that owns LIVECHAT_PAT`);
            er.status=422; er.code='LIVECHAT_REQUESTER_USER_ID_INVALID'; er.retryable=false; er.cause=privateErr; throw er;
          }
          const er=new Error(`LIVECHAT_PUBLIC_AGENT_LIMIT: provider rejected both public and private requester membership (${String(privateErr?.message||privateErr).slice(0,240)})`);
          er.status=422; er.code='LIVECHAT_PUBLIC_AGENT_LIMIT'; er.retryable=false; er.cause=privateErr; throw er;
        }
      }
